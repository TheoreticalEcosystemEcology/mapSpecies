...
meshDom1 <- inla.mesh.2d(loc.domain = MEGR01, max.edge = 2,
                         offset = c(1,1), crs = CRS("+proj=longlat +datum=NAD27"))
meshDom2 <- inla.mesh.2d(loc.domain = MEGR01, max.edge = 0.6,
                         offset = c(1,1))
meshDom3 <- inla.mesh.2d(loc.domain = MEGR01, max.edge = 0.5,
                         offset = c(0.5,0.5))
```
...
From this point on, we will use `meshDom3` for this illustration. 
...
Let's build a logistic, a probit and a complementary log-log model for the presence-absence data. Note that, we included the argument `control.compute`, which is an argument from INLA, to compute a Watanabe-Akaike information criterion (WAIC) to use as a way to compare the different models that will be considered here. 
...
modelProbit <- uniSpace(MEGR ~ ., spdf = MEGR01,
                        explanaMesh = explana, family = "binomial",
                        link = "probit", 
                        control.compute = list(waic = TRUE))

modelCLogLog <- uniSpace(MEGR ~ ., spdf = MEGR01,
                         explanaMesh = explana, family = "binomial",
                         link = "cloglog", 
                         control.compute = list(waic = TRUE))
```

Because the WAIC was calculated for the different model, we can compare the model to decide which one is the best to use. 

```{r}
modelLogit$waic$waic
modelProbit$waic$waic
modelCLogLog$waic$waic
```

From the WAIC, we can see that the complementary log-log model is the best one (lowest WAIC). From this point on, we will focus on this model.

### Studying the estimated parameters

Although one of the main interest of species distribution models is to estimate and predict the distribution of species, it is also relevant and important to study the estimated parameters to better understand the importance of the considered explanatory variable in structuring the distribution of the species.

```{r}
modelProbit$model$summary.fixed
```
