...
test_that("PP spatial model works", {
  if(FALSE){
    require(spatstat)
...
test_that("PA spatial model works", {
  
  xy <- matrix(runif(20),ncol=2)
  resp <- data.frame(resp = rbinom(10, size = 1, prob = 0.3))
  spdf <- SpatialPointsDataFrame(coords = xy, data = resp)
