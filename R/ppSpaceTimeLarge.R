...
#' 
#' @keywords models
ppSpaceTimeLarge <- function(formula, y, weightPP, explanaMesh, 
                             meshTime, smooth = 2,
  ...
  refData <-  data.frame(y = 1,values(dataPrep$X))
  ...
  xyXorg <- cbind(coordinates(dataPrep$X),Xorg)
  ...
  res <- list(model = model, Stack = Stack, 
              meshSpace = dataPrep$mesh, meshTime = meshTime)
